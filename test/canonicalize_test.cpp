...
TEST_CASE("BranchCanonicalizer if-else", "[dcec][if]") {
    ...
    auto ExpectedCode = R"code(int foo(int a){
        if (a > 0){
            a = 1;
        } else{
            a = 0;
    ...
    CAPTURE(Code);
    REQUIRE(formatCode(ExpectedCode) == runBranchCanonicalizerOnCode(Code));

    Code = R"code(int foo(int a){
            if (a > 0){
                a = 1;
            }
            else
...
TEST_CASE("BranchCanonicalizer loops",
          "[dcec][loop][for][while][do][range-for]") {
    std::string Code = R"code(#include<initializer_list>
    int foo(int a){
        LOOP
            a = 0SPACE;END
        return a;
    ...
    std::string ExpectedCode = R"code(#include<initializer_list>
    int foo(int a){
        LOOP{
            a = 0;
        }END
...
TEST_CASE("BranchCanonicalizer switch", "[dcec][switch]") {
    ...
    std::string ExpectedCode = R"code(int foo(int a){
        switch(a){
            case 1:{
                break;
            }
...
TEST_CASE("BranchCanonicalizer empty body", "[dcec][if][for][do]") {
    std::string Code = R"code(int foo(int a){
        if (a > 0);
        for(;;);
        do;while(1);
...
TEST_CASE("BranchCanonicalizer nested if", "[dcec][if][nested]") {
    std::string Code = R"code(int foo(int a){
        if (a > 0)
            if (a == 10)
                return 10;
...
TEST_CASE("BranchCanonicalizer nested complex",
